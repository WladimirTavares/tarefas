...
char* htmlFeedBack(int ncts, char* supdirct, char* supdirctresp) {
    ...
    // abrir arquivo de avaliação global da tarefa
    char caminho[4097] = "";
    ...
    int resultadotarefa;
    fscanf(fat, "%d", &resultadotarefa);

    if (resultadotarefa) {
        sprintf(linha, "<div>Testes bem sucedidos!</div><br>");
    }
    else {
        sprintf(linha, "<div>Falha nos testes.</div><br>");
    }
