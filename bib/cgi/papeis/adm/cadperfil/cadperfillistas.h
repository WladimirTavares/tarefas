...
typedef struct LinhaPerfilLote {
    ...
    //int perfilExiste;
    int cadastrar;                  // indica que o usuário optou por cadastrar o perfil
    int incluirRespondedor;         // indica que o usuário optou por incluir o respondedor na turma


    int perfilPreExistente;         // indica que o perfil existia antes da tentativa de cadastro
    int loginPreExistente;          // indica que o login existia antes da tentativa de cadastro
    //int ignorado;                   // indica que o usuário não selecionou o perfil para cadastro mas tinha

    int respondedorIncluido;

    Usuario* perfilExistente;
...
typedef struct LinhaRelCadPerfil {
	int numerr;        // Número de erro:       1 - linha nula
                       ...
                       //                       31 - perfil repetido
                       //                       41 - dados de novo usuário inválidos
                       //                       111 a  - Erro na fase de cadastro em lote
...
typedef struct RelatorioCadPerfil {
    ...
    int status;                 // Número de erro:      1 - Linha nula, problema na leitura do arquivo
                                ...
                                //                      9 - Perfil repetido
                                //                      10 - Dados de novo usuário inválidos
                                //                      11 - Erro na fase de cadastro em lote
...
int rcpEsvaziarRelatorio(RelatorioCadPerfil* rel);
...
LinhaPerfilLote* lplCriarLinha() {
    ...
    linhapl->cadastrar = 0;
    linhapl->incluirRespondedor = 0;

    //linhapl->ignorado = 0;
    linhapl->loginPreExistente = 0;
    linhapl->perfilPreExistente = 0;

    linhapl->respondedorIncluido = 0;

    linhapl->perfilExistente = NULL;
...
LinhaRelCadPerfil* rcpConstruirLinha(int numerr, char* msg, LinhaPerfilLote* lpl) {
    ...
    linha->lpl = lpl;
    //linha->perfilExistente = perfilExistente;
...
void rcpImprimir(LinhaRelCadPerfil* linha) {
    if (linha){
        ...
        printf("numerr: %d, msg: %s, perfilExistente: %p", linha->numerr, linha->msg, linha->lpl->perfilExistente);
    }
}
